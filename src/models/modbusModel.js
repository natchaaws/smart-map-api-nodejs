...
module.exports = {
  ...
  async getMessagesByModbusId(modbusId, offset, perPage, searchWord) {
    try {
      ...
      const dataQuery = `
                SELECT 
                    id, name, message,
                    LPAD(CAST(length_count AS TEXT), 6, '0') AS length_count, 
                    x, y, 
                    modbus_id, 
  ...
  async findDevicesByIpPort(ip, port) {
    const query = `
            SELECT modbus_id, modbus_ip, port, websocket_type, length, start, status, image, created_by, created_date, modified_by, modified_date, deleted_by, deleted_date, is_delete 
            FROM public.modbus_devices
            WHERE modbus_ip = $1 AND port = $2 AND is_delete = false
            ORDER BY modbus_id ASC
        `;
    const values = [ip, port];
    const result = await pool.query(query, values);

    return result.rows; // คืนข้อมูลที่ค้นพบ
  },
  ...
  async updateDevice(modbusId, data) {
    const query = `
        UPDATE public.modbus_devices
        SET 
            modbus_ip = COALESCE($1, modbus_ip),
            port = COALESCE($2, port),
            websocket_type = COALESCE($3, websocket_type),
    ...
    const result = await pool.query(query, values);
    if (result.rows.length === 0) throw new Error("Modbus device not found");
    return result.rows[0];
  },
  ...
  async deleteLastMessages(modbusId, count) {
    const query = `
        ...
        `;
    const result = await pool.query(query, [modbusId, count]);
    return result.rowCount; // คืนค่าจำนวนแถวที่ได้รับการอัปเดต
  },
  ...
  async updateMessagesStart(modbusId, newStart) {
    const query = `
        ...
        SET 
            ...
            name = 'Address-' || rm.new_length_count::text
        FROM 
            ranked_messages AS rm
        WHERE 
            mm.id = rm.id;
    `;
    const result = await pool.query(query, [newStart, modbusId]);
    return result.rowCount; // คืนค่าจำนวนแถวที่ได้รับการอัปเดต
  },

  // ฟังก์ชันเพื่อตรวจสอบจำนวน modbus_messages ใน modbus_id
  async countMessagesByModbusId(modbusId) {
    const query = `
        SELECT COUNT(*) AS total
        FROM public.modbus_messages
        WHERE modbus_id = $1
  ...
  async showMessagesByModbusId(modbusId) {
    const query = `
        SELECT *
        FROM public.modbus_messages
        WHERE modbus_id = $1
